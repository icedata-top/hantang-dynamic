 ...
 */
export function getActionName(action: UserRelationAction): string {
  switch (action) {
 ...
 */
export function getDefaultFilename(action: UserRelationAction): string {
  switch (action) {
    case UserRelationAction.Follow:
      return "follow.csv";
    case UserRelationAction.Unfollow:
 ...
 */
export async function processUserRelations(
  ...
  const defaultFileName = getDefaultFilename(actionType);
  ...
  // Read CSV file
  logger.info(`Reading user data for ${actionName} operation from ${filePath}`);
