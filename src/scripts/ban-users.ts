...
async function main() {
  logger.info("Starting batch block process");
  logger.info(`Parameters: CSV=${csvPath || "data/block.csv"}, Batch Size=${batchSize}, Wait Time=${waitTime}ms`);
  
  try {
    await processFollows(csvPath, batchSize, waitTime, false);
  } catch (error) {
