...
async function fetchVideos(): Promise<void> {
  ...
  if (newVideosResponse.data.cards?.length) {
    const latestDynamicId =
      newVideosResponse.data.cards[0].desc.dynamic_id.toString();
    console.log(
      `Found ${newVideosResponse.data.cards.length} new videos. Latest dynamic ID: ${latestDynamicId}`
    );
  ...
  let currentOffset =
    newVideosResponse.data.next_offset ||
    newVideosResponse.data.cards?.[
      newVideosResponse.data.cards.length - 1
    ]?.desc.dynamic_id?.toString();
  let pageCount = 1;
  ...
  while (currentOffset) {
    ...
    console.log(
      `Retrieved ${historyResponse.data.cards.length} videos on page ${pageCount}`
    );

    // Check if the oldest video in this batch is too old
    const oldestVideo =
      historyResponse.data.cards[historyResponse.data.cards.length - 1];
    ...
    if (!stateManager.isWithinMaxHistory(oldestPubdate)) {
      console.log(
        `Oldest video (${oldestDate.toISOString()}) is beyond max history days (${
    ...
    if (foundLastVideo) {
      console.log(
        `Found last known video (dynamic ID: ${lastDynamicId}). Stopping.`
      );
      break;
    }
