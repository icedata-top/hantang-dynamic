...
本文只覆盖 `hantang-dynamic` 接手 `daily_delta > 100` 视频池的 V1 minute 采集闭环。大盘指数、Java 后端只读 API、React 前端展示和大盘弱修正不属于 V1，统一放在 `docs/plans/postgres-market-index-plan.md`。
...
`priority = -2` 和 `priority = 0` 需要双向调整：`priority = 0` 的视频如果连续 `weekly_zero_delta_days` 个完整自然日总新增播放量为 0，就降为 `-2`；`priority = -2` 的视频只要周日 daily 发现最近窗口出现正增长，就升回 `0`，后续再按 daily delta 规则决定是否升为正数 minute 周期。
...
5. 成功 minute 采样后，由 PostgreSQL 根据新写入的 `video_minute` 事实行更新 `last_minute_success_at`、`last_view` 和下一次 `next_minute_due_at`。queue 完成只通过 `ack_video_collection_tasks(task_ids, lease_token)` 处理。
...
5. 关口任务成功后同样写入 `video_minute`，再调用 `ack_video_collection_tasks(task_ids, lease_token)` 完成当前 gate task。
...
3. 普通任务完成不要求 TS 显式逐行更新 state。TS 插入 `video_minute` 后调用 `ack_video_collection_tasks(task_ids, lease_token)`。queue 完成只走该函数，并按 task id、lease token 和任务状态校验，避免只按 AID 误完成非本轮任务。
...
8. 成功 aid 的 state 推进由 PostgreSQL 函数或 `video_minute` 写入 trigger 完成，计算口径为 `next_minute_due_at = last_minute_success_at + priority minutes`。queue 完成只由 worker 调用 `ack_video_collection_tasks(task_ids, lease_token)`。
...
5. `priority = 0` 的视频最近 7 个完整自然日新增播放量为 0 时，PostgreSQL 只把 `priority` 调整为 `-2`，不新增 cadence 字段。
