...
5. SaaS crawler 不再处理这批视频的 minute 任务。
...
`priority = -2` 和 `priority = 0` 需要双向调整：`0` 连续 `weekly_zero_delta_days` 个完整自然日总新增播放量为 0 后降为 `-2`；`-2` 只要周日 daily 发现最近窗口新增播放量大于 0，就升回 `0`，后续再按 daily delta 规则决定是否升为正数 minute 周期。
...
5. 关口任务成功后写入 `video_minute`，再调用 `ack_video_collection_tasks(task_ids, lease_token)` 完成当前 gate task。
...
1. `src/database/videoDaily.ts` 或等价 helper，提供 `daily_delta > 100` 候选查询。
...
fn_ack_video_collection_tasks(p_task_ids bigint[], p_lease_token uuid)
```
...
1. 增加 `daily_delta > 100` 候选 helper。
...
8. 批量写 `video_minute`。
9. 成功 aid 通过 PostgreSQL 函数或 trigger 推进 state，queue 完成只由 worker 调用 `ack_video_collection_tasks(task_ids, lease_token)`，失败 aid 只写日志并等待重试。
...
4. `src/services/minute/metrics.ts`
5. 独立 minute worker 进程入口
6. `market_*` 指数表
