...
1. 用 `daily_delta > 100` 建立 minute 采集观察池。
...
7. 成功 aid 的 state 推进由 PostgreSQL 函数或 `video_minute` 写入 trigger 处理，queue 完成只走 `ack_video_collection_tasks(task_ids, lease_token)`。
...
10. `priority = 0` 的视频最近 7 个完整自然日新增播放量为 0 时，PostgreSQL 只把 `priority` 调整为 `-2`。
...
5. `priority = 0` 的视频最近 7 个完整自然日新增播放量为 0 时，只调整 `priority` 为 `-2`。
...
3. `priority = 0` 连续 7 个完整自然日总新增为 0 后降为 `-2`。
